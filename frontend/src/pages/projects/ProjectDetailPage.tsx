...
import { DeleteOutlined, DownloadOutlined, EditOutlined, FileZipOutlined, FolderAddOutlined, InboxOutlined, LockOutlined, SearchOutlined, ShareAltOutlined, UploadOutlined } from '@ant-design/icons'
import type { UploadFile } from 'antd'
import { useNavigate, useParams, Link } from 'react-router-dom'
import { createProjectFolder, deleteProject, deleteProjectFile, downloadProjectFile, getProjectDetail, getProjectDirectory, getProjectFilePermissions, getProjectFiles, retryProjectFileParse, updateProject, updateProjectFilePermission, uploadProjectFile, type FilePermissionMember, type ProjectDetail, type ProjectFile, type ProjectFolder } from '@/api/workspace'
...
export function ProjectDetailPage() {
  ...
  const renderFileRow = (file: ProjectFile) => {
    ...
    return <div key={file.id} className="hd-asset-row" role={canPreview ? 'button' : undefined} tabIndex={canPreview ? 0 : undefined} onClick={() => canPreview && navigate(`/files/${file.id}/preview`)}>
      ...
      <div className="hd-asset-actions">{canPreview && project?.permission === 'edit' ? <button type="button" className="hd-icon-btn" title="管理分享" aria-label={`管理 ${file.name} 的分享链接`} onClick={(event) => { event.stopPropagation(); navigate(`/files/${file.id}/preview?share=manage`) }}><ShareAltOutlined /></button> : null}<button type="button" className="hd-icon-btn" title="下载原型" aria-label={`下载 ${file.name}`} onClick={(event) => { event.stopPropagation(); void downloadProjectFile(project!.id, file.id).catch(() => message.error('下载失败')) }}><DownloadOutlined /></button>{project?.permission === 'edit' ? <button type="button" className="hd-icon-btn" title="文件权限" aria-label={`管理 ${file.name} 的文件权限`} onClick={(event) => { event.stopPropagation(); void openPermissionManager(file) }}><LockOutlined /></button> : null}{file.canDelete ? <button type="button" className="hd-icon-btn" title="删除原型" aria-label={`删除 ${file.name}`} onClick={(event) => { event.stopPropagation(); handleDeleteFile(file) }}><DeleteOutlined /></button> : null}</div>
    </div>
  }
