...
- **分支：** main
- **最新提交：** `041285e fix(viewer): 修复 Axure 页面顺序与层级显示`
- **三端对齐：** 2026-09-14 14:40 已通过离线 Git bundle 将服务器部署到最新代码；本地、GitHub、服务器 HEAD 均为 `041285e700b3a1de7d2a599bd1fecf2a35af6252`，服务器工作区干净。

### 部署状态
- **线上环境：** http://47.114.41.30:8080/
- **部署方式：** 阿里云 Workbench CLI，经离线 Git bundle 上传，不依赖服务器直连 GitHub。
- **最后部署：** 2026-09-14 14:40 GMT+8（Workbench CLI 自动化部署）
- **健康状态：** 
  - API: ✅ ok
  - MySQL: ✅ Healthy
  - Redis: ✅ ok
...
- **构建产物验证：**
  - API 镜像: ✅ 构建完成
  - Web 镜像: ✅ 构建完成（29 个资源文件）
  - 数据库迁移: ✅ 完成（5 个迁移已应用，0 个待处理）
- **关键功能：** 已包含超级管理员全局数据权限与账号管理、分享链接权限类型、分享链接随时复制与历史链接重新生成、团队邀请链接。
...
## 七、2026-09-11 团队邀请链接迭代

### 已完成
- 团队详情页“邀请成员”按钮已启用，管理员可创建并复制团队邀请链接。
- 受邀人可访问 `/team-invites/:token` 查看团队信息；登录或注册后会自动完成加入。
- 新增 `TeamInvite` 数据模型与 Prisma 迁移 `20260911134000_add_team_invites`，提供创建、查询、接受与撤销接口。

### 关键规则
- 链接令牌使用高熵随机值，数据库只保存 SHA-256 哈希。
- 链接有效期为 7 天；仅团队管理员或超级管理员可创建或撤销。
- 接受者成为普通成员，默认无上传权限；重复接受不会重复创建成员关系。

### 验证
- Prisma Client 已生成；后端 Nest 构建通过。
- 前端 TypeScript 检查与 Vite 生产构建通过。
- 团队邀请服务单测 4/4 通过。

### 发布提示
- 上线时需执行新增 Prisma 迁移，并按既有 Workbench 部署流程同步发布前后端。
